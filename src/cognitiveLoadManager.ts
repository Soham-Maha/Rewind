...
export class CognitiveLoadManager {
    ...
    private getOrInitLineMetrics(file: string, linesCount: number): LineMetrics[] {
        if (!this.fileMetrics.has(file)) {
            const arr = Array.from({ length: linesCount }, () => this.createEmptyMetrics());
    ...
    public onDocumentChange(event: vscode.TextDocumentChangeEvent) {
        ...
        for (const change of changes) {
            ...
            if (deletedLines > 0 || addedLines > 0) {
                ...
                const newItems = Array.from({ length: addedLines }, () => this.createEmptyMetrics());
            ...
            metrics[startLine].totalScore = this.strategy.calculateScore(metrics[startLine]);
        }
    }
    ...
    private startDwellTracking(file: string, line: number, documentLineCount: number) {
        ...
        this.dwellTimer = setInterval(() => {
            if (this.currentCursorFile && this.currentCursorLine !== null) {
                const metrics = this.getOrInitLineMetrics(this.currentCursorFile, documentLineCount);
                if (this.currentCursorLine < metrics.length) {
                    ...
                    metrics[this.currentCursorLine].totalScore = this.strategy.calculateScore(metrics[this.currentCursorLine]);
