...
export function updateHeatmapDecorations(editor: vscode.TextEditor, manager: CognitiveLoadManager | null) {
    ...
    const decOptions: vscode.DecorationOptions[][] = Array.from({ length: GRADIENT_STEPS + 1 }, () => []);
    ...
    for (let i = 0; i < metricsArr.length; i++) {
        ...
        hoverMessage.appendMarkdown(`**Cognitive Load**: ${metrics.totalScore.toFixed(1)} *(Blended: ${smoothedScore.toFixed(1)})*\n\n`);
        hoverMessage.appendMarkdown(`- Edits: ${metrics.editCount}\n`);
        hoverMessage.appendMarkdown(`- Deletions: ${metrics.deletions}\n`);
        hoverMessage.appendMarkdown(`- Dwell Time: ${(metrics.dwellTimeMs / 1000).toFixed(1)}s\n`);

        const decOpt = { range, hoverMessage };

        // Linear mapping up to GRADIENT_STEPS (score of 20 = purely red max)
        const decIndex = Math.max(0, Math.min(GRADIENT_STEPS, Math.floor(smoothedScore)));
